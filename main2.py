...
def create_pass():
    password1 = [random.choice(letters) for _ in range(random.randint(2, 3))]
    password2 = [str(random.randint(0, 9)) for _ in range(random.randint(2, 4))]
    password3 = [random.choice(symbols) for _ in range(random.randint(2, 5))]
    password4 = [random.choice(letters).upper() for _ in range(random.randint(2, 3))]

    combined_password = password1 + password2 + password3 + password4
...
def on_entry_focus_in(event):
    event.widget.config(bg='#E3F6FF')


def on_entry_focus_out(event):
    event.widget.config(bg='#FFFBEB')
...
app.config(bg="#BEF0CB", padx=40, pady=2)
...
def add_new_letter():
    ...
    if x < height:
        ...
        label2 = tkinter.Label(app, text=letter_num_string, font=("Arial", 12), bg="white", fg=colors)
        label2.grid(row=2, column=0, columnspan=3)
        label2.update()
        app.after(50, add_new_letter)
...
# timer_text = canvas.create_text(100, 100, fill="white")
...
button_pass_add = Button(text="")
button_pass_add.grid(row=0, column=0, columnspan=3)
button_pass_add = Button(text="")
button_pass_add.grid(row=2, column=2, columnspan=3)
...
button_pass_gen.grid(row=4, column=2, sticky="ew")
